...
export function renderTranslatorPage(app: HTMLDivElement): void {
    ...
    <main class="translator-page">
      ...
      <section class="translator-grid">
        <article class="section-card">
          ...
          <ul class="translator-bullets">
            ...
            <li><code>g_plan_table</code> tells what high-level action the robot should do next.</li>
      ...
      <section class="translator-grid">
        ...
        <article class="section-card">
          ...
          <p class="hero-copy">If a teammate already copied one row from <code>generated_plan_table.c</code>, paste it here to decode the action list.</p>
  ...
  function updateManualPlacementResult(): void {
    ...
    const plan = buildPlanForPlacement(placement);
    const steps = buildTutorialExecution(plan);
    placementStatus.textContent = `Matched legal layout ${layoutId}. This is the same layout-to-plan relationship used by the generated STM32 tables.`;
    renderTutorialResult(
      manualResult,
      "Manual Placement Tutorial",
      placement,
